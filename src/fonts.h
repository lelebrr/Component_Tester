...
// Tabela de fonte 5x7 clássica (ASCII 32-127)
...
inline void draw_text_5x7(TFT_eSPI& tft, int16_t x, int16_t y, const char* str, uint16_t color, uint8_t size) {
    while (*str) {
        ...
        for (int8_t i = 0; i < 5; i++) { // 5 colunas por caractere
            uint8_t line = pgm_read_byte(&font5x7[(c - 32) * 5 + i]);
            for (int8_t j = 0; j < 7; j++) { // 7 pixels por coluna
                if (line & (1 << j)) {
                    if (size == 1) tft.drawPixel(x + i, y + j, color);
                    else tft.fillRect(x + i * size, y + j * size, size, size, color);
                }
            }
        }
        x += 6 * size; // 5 pixels + 1 de espaço
    }
}
